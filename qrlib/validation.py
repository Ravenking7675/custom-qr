...
from exceptions_here import (StyleMissing, InvalidColor, InvalidSize,
...
def ec_level_validation(ec_level):
    if not isinstance(ec_level, str) or \
...
def format_validation(qr_format):
    if not isinstance(qr_format, str) or \
...
def application_validation(application):
    if not isinstance(application, str) or \
...
def appsize_validation(app_size):
    if not isinstance(app_size, str) or \
